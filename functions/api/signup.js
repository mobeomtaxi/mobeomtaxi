...
export async function onRequestPost({ request, env }) {
  try {
    ...
    // 아이디 중복 체크
    const existsUser = await env.DB.prepare(`SELECT id FROM users WHERE username = ?`)
      .bind(username)
      .first();
    ...
    // 닉네임 중복 체크
    const existsNick = await env.DB.prepare(`SELECT id FROM users WHERE nickname = ?`)
      .bind(nickname)
      .first();
    ...
    const res = await env.DB.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?)
    `)
      .bind(username, password_hash, nickname, email, phone, intro, recommender)
      .run();
...
// PBKDF2 해시 (Cloudflare에서 iterations 100000 초과하면 NotSupportedError 날 수 있음)
async function hashPassword(password) {
  const iterations = 100000; // ✅ 안정값
